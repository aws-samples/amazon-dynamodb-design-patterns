...
Import `AnOnlineShop_1.json` to create a new data model in NoSQL Workbench called `AnOnlineShop` as well as a new table called `OnlineShop`.
...
Import `AnOnlineShop_2.json` to handle the below access pattern:
...
Import `AnOnlineShop_3.json` to respond to the following access pattern about the Product entity:
...
To handle the following access pattern for the Warehouse entity, import `AnOnlineShop_4.json`:
...
Let's continue by importing `AnOnlineShop_5.json`:
...
Let's populate the table with some more Customer, Product and Warehouse items by importing `AnOnlineShop_6.json`.

### Step 7

Import `AnOnlineShop_7.json` to build an item collection for Order that can respond to the following access patterns:
...
Import `AnOnlineShop_8.json` to add an Invoice entity to the Order item collection to handle this access pattern:
...
Import `AnOnlineShop_9.json` to add Shipment entities to Order item collection:
...
In the previous steps, an item collection for Order was created. Import `AnOnlineShop_10.json` to create a new item collection that makes it possible to retrieve OrderItem data from several Order item collections. The first global secondary index (GSI) is created for this reason. 
...
Import `AnOnlineShop_11.json` to handle the following access patterns related to Invoice. As can be seen, even though there are two different access patterns, they are realized using the same key condition. Payments are defined as an attribute with Map data type on the Invoice entity.
...
Import `AnOnlineShop_12.json` to handle the following access patterns. 
...
Import `AnOnlineShop_13.json` to model the final access patterns:
...
Import `AnOnlineShop_facets.json` to see the [facets](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/workbench.Visualizer.Facets.html) for the different access patterns of this use case.
...
Import `AnOnlineShop_14.json` to check the revised data model. Notice that the prefix from the date string is removed and filter expression is used instead. The new access pattern is handled as below:
